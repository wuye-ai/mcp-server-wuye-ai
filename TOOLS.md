...
| `物业问题场景分析`          | `AnalyzeScene`                                | 物业问题场景分析：对于物业行业垂直的专业问题，让 CRIC物业AI 帮助分析问题的场景，判断应搜索的知识库。较为简单直白，或者不是专属于物业行业的问题，不适用这个工具。                                         | `query`(问题)、`is_complicated`(问题是否涉及物业行业多个领域或不同形式)、`context_materials`(其他有助于分析问题的信息。请提供你认为和问题相关的文本，形式为文本数组。) 等                        | ✅   | ❌    | ❌       | ❌      | ❌   |
